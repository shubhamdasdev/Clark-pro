...
Ground remains open, and the permanent Mac implementation has started. The executable Studio/Harness slice now has durable SQLite/WAL events and recovery, content-addressed artifacts, exact-version approval, a real source-pinned zero-egress MCP client call with leases/receipts, and an authenticated one-workspace Clark Bridge MCP server. Both connections are visible in the sandboxed Mac shell and run from the packaged-ASAR topology. This proves a narrow production path, not the full creator OS.

The OpenCut candidate remains deliberately upstream-blocked rather than presented as a working integration. Models, third-party Tool Pack/skill installation, governed memory, social accounts and publishing, remote workers/team sync, broad cancellation/chaos coverage, signed/notarized distribution, observed creator/commercial evidence, native Mac accessibility review, and leadership signoff are not yet proven.
